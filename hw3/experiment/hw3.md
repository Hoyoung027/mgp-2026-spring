...
현재 baseline은 `Step 1: thread당 output 2개 계산`을 도입한 버전이다. 채점 서버 환경에서 아래 조건으로 측정했다.
...
- 최고 성능 기준: `0.714 ms -> 0.672 ms`
- 최악 성능 기준: `0.727 ms -> 0.688 ms`
- 평균 성능 기준: `0.720 ms -> 0.682 ms`
- 정확도: 동일하게 `0.005859`

채점은 5회 중 최대 실행 시간을 기준으로 하므로, 가장 중요한 비교값은 최악 성능이다. 현재 baseline은 worst-case 기준으로 약 `5.36%` 개선되었다.
...
- 성능 기준: 최악 실행 시간 `0.688 ms`보다 빨라야 한다.
